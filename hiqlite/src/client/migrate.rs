...
impl Client {
    ...
    ///
    /// To make this work, you currently need to `Embed` a `struct` in your app to be able to
    /// execute them in a type-safe way:
    ///
    /// ```rust, notest
    ...
    /// # Panics
    ///
    /// If the given Migrations have an invalid format.
    pub async fn migrate<T: RustEmbed>(&self) -> Result<(), Error> {
        let migrations = Migrations::build::<T>();
        self.migrate_execute_dedup::<T>(migrations).await
    }

    /// Execute database migrations.
    ///
    /// This version throws an error instead of a `panic` on invalid migrations.
    ///
    /// To make this work, you currently need to `Embed` a `struct` in your app to be able to
    /// execute them in a type-safe way:
    ///
    ...
    /// You might want to take a look at the
    /// [sqlite-only](https://github.com/sebadob/hiqlite/tree/main/examples/sqlite-only) example.
    pub async fn try_migrate<T: RustEmbed>(&self) -> Result<(), Error> {
        let migrations = Migrations::try_build::<T>().map_err(Error::Sqlite)?;
        self.migrate_execute_dedup::<T>(migrations).await
    }

    #[cold]
    pub(crate) async fn migrate_execute_dedup<T: RustEmbed>(
        &self,
        ...
        match self.migrate_execute(migrations).await {
            ...
            Err(err) => {
                if self
                    ...
                    .await
                {
                    // cannot fail if validation was fine before
                    self.migrate_execute(Migrations::try_build::<T>().unwrap())
                        .await
                } else {
                    Err(err)
                }
